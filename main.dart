...
class TodayAttendanceCards extends StatelessWidget {
  ...
  Widget _buildAttendanceList(
    ...
    final presentCount =
        sortedRecords.where((record) => record['status'] == 'Present').length;
...
class _MarkAttendanceDialogState extends State<MarkAttendanceDialog> {
  ...
  Future<void> _addNewStudent() async {
    ...
    try {
      final docSnapshot =
          await _firestore.collection('Student').doc(studentName).get();
                ...
                children: [
                  Expanded(
                    child: ElevatedButton(
                      onPressed:
                          _isLoading ? null : () => _markAttendance('Present'),
                      child: const Text(
                        'Present',
                        style: TextStyle(
                          fontSize: 15,
                  ...
                  Expanded(
                    child: ElevatedButton(
                      onPressed:
                          _isLoading ? null : () => _markAttendance('Absent'),
                      child: const Text(
                        'Absent',
                        style: TextStyle(
                          fontSize: 15,
